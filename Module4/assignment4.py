...
Plot2D(T,'2D graph',0,1,num_to_plot=100)
...
def Plot3D(T, title, x, y,z, num_to_plot=40):
  ...
  ax.set_xlabel('Component: {0}'.format(x))
  ax.set_ylabel('Component: {0}'.format(y))
  ax.set_ylabel('Component: {0}'.format(z))
  x_size = (max(T[:,x]) - min(T[:,x])) * 0.08
  y_size = (max(T[:,y]) - min(T[:,y])) * 0.08
  ...
  for i in range(num_to_plot):
    ...
    img = df.iloc[img_num,:].reshape(num_pixels, num_pixels)
    ax.imshow(img, aspect='auto', cmap=plt.cm.gray, interpolation='nearest', zorder=100000, extent=(x0, x1, y0, y1,z0,z1))
  
  ax.scatter(T[:,x],T[:,y],T[:,z],marker='.',alpha=0.7)

Plot3D(T, 'blah', 0, 1,2, num_to_plot=40)
